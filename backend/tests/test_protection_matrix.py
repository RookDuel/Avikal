...
from avikal_backend.archive.security.pqc_keyfile import PQC_EMBEDDED_MEMBER_NAME, PQC_STORAGE_MODE_EMBEDDED
...
def _create_archive(
    *,
    ...
    if pqc_enabled and result["pqc"].get("storage_mode") != PQC_STORAGE_MODE_EMBEDDED:
...
def test_single_embedded_pqc_roundtrip_uses_archive_member():
    with _workspace_tempdir() as temp_dir:
        archive_path, expected, pqc_keyfile_path = _create_archive(
            archive_kind="single",
            root=temp_dir,
        ...
        assert pqc_keyfile_path is None

        from zipfile import ZipFile

        with ZipFile(archive_path, "r") as zf:
            assert PQC_EMBEDDED_MEMBER_NAME in zf.namelist()
        ...
        extracted = _collect_extracted_outputs(output_dir, "single", extraction_result)
        assert extracted == expected
