...
from avikal_backend.archive.format.metadata import METADATA_FORMAT_VERSION_ASSURED
...
def test_rekey_rotates_password_without_rewriting_payload():
    ...
    try:
        ...
        assert metadata_before["version"] == METADATA_FORMAT_VERSION_ASSURED
        ...
        assert metadata_after["version"] == METADATA_FORMAT_VERSION_ASSURED
        ...
        with pytest.raises(ValueError, match="Incorrect password|Chess decoding failed"):
            extract_avk_file(str(archive), str(output_dir), password="OldPass#1234")

        extracted = Path(extract_avk_file(str(archive), str(output_dir), password="NewPass#4567"))
