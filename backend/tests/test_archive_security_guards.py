...
def test_archive_input_rejects_symbolic_links():
    with _workspace_tempdir() as temp_dir:
        target = temp_dir / "target.txt"
        link = temp_dir / "linked.txt"
...
def test_tampered_protected_payload_fails_authentication_before_write():
    with _workspace_tempdir() as temp_dir:
        payload = temp_dir / "payload.txt"
        archive = temp_dir / "protected.avk"
        tampered_archive = temp_dir / "protected_tampered_payload.avk"
        output_dir = temp_dir / "out"
        output_dir.mkdir()
        payload.write_bytes(b"protected payload\n" * 16)
        ...
        header_bytes, keychain_pgn, payload_bytes = read_avk_container(str(archive))
        tampered_payload = bytearray(payload_bytes)
        tampered_payload[-1] ^= 0x01
        _rewrite_archive(tampered_archive, keychain_pgn=keychain_pgn, payload_bytes=bytes(tampered_payload))

        with pytest.raises(ValueError, match="authentication failed|Merkle verification failed"):
            extract_avk_file_enhanced(
                str(tampered_archive),
                str(output_dir),
                password=PASSWORD,
        ...
        assert list(output_dir.iterdir()) == []
        assert list(output_dir.glob(".avikal-*")) == []
...
def test_plaintext_archive_tampered_filename_cannot_escape_output_directory():
    with _workspace_tempdir() as temp_dir:
        ...
        with pytest.raises(ValueError, match="unsafe filename|path separators"):
            encode_metadata_to_chess_enhanced(
                tampered_metadata,
                password=None,
                keyphrase=None,
